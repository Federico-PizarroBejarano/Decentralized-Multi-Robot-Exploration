        ...
        for probability_of_failed_communication in [0, 50, 80, 100]:
            for probability_of_failed_scan in [10]:
                for run in range(runs):
                    try:
                        obs,pose = eval_world.reset('test_{}.npy'.format(map_id), all_starting_poses[starting_poses_key], probability_of_failed_communication, probability_of_failed_scan)
                        pose = th.tensor(pose)
